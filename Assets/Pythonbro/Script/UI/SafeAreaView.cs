...
public class SafeAreaView : MonoBehaviour {
    ...
    // 设计分辨率
    public float matchWidth = 2160f;    // UI单位
    public float matchHeight = 1080f;   // UI单位

    public float minRatio = 16f / 10f;    // 限制最小宽高比
    public float maxRatio = 18f / 9f;   // 限制最大宽高比
    ...
    private void ResetSafeArea() {
        ...
        float scale = safeArea.width / Screen.width;    // 整体缩放值
    ...
    private IEnumerator ResetMaskArea(Vector2 areaPos, Vector2 areaSize, Vector3 areaScale) {
        ...
        foreach(RectTransform maskArea in maskAreas) {
            ...
            float left = (maskSize.x - areaSize.x + areaPos.x) * 0.5f;
            float right = (maskSize.x - areaSize.x - areaPos.x) * 0.5f;
