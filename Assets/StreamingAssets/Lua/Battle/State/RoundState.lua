...
setmetatable(RoundState, FSMachine.State)

function RoundState:Create(battle)
    local copy = {}
    setmetatable(copy, RoundState)
    copy.battle = battle
...
function RoundState:OnEnter(orderHero)
...
function RoundState:DoNext()
    ...
    if self.battle.nowActionHero == nil then
        self.battle.nowActionHero = self.orderHero[1]
    else
        for index, hero in ipairs(self.orderHero) do
            if hero == self.battle.nowActionHero then
                self.battle.nowActionHero = self.orderHero[index + 1]
                break
            end
        end 
    end
...
--                     if heroData ~= nil and not heroData.isDead and not heroData.isOwn and not self:HpIsFull(heroData) then
--                         return skillId
--                     end
--                 end
--             end
