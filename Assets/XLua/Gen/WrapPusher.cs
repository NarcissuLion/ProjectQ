...
namespace XLua
{
    public partial class ObjectTranslator
    {
        ...
        class IniterAdderUnityEngineVector2
        {
			...
			static void Init(LuaEnv luaenv, ObjectTranslator translator)
			{
				...
				translator.RegisterPushAndGetAndUpdate<ImageAspectRatioFitter.AspectMode>(translator.PushImageAspectRatioFitterAspectMode, translator.Get, translator.UpdateImageAspectRatioFitterAspectMode);
        ...
        int ImageAspectRatioFitterAspectMode_TypeID = -1;
		int ImageAspectRatioFitterAspectMode_EnumRef = -1;
        
        public void PushImageAspectRatioFitterAspectMode(RealStatePtr L, ImageAspectRatioFitter.AspectMode val)
        {
            if (ImageAspectRatioFitterAspectMode_TypeID == -1)
            {
			    bool is_first;
                ImageAspectRatioFitterAspectMode_TypeID = getTypeId(L, typeof(ImageAspectRatioFitter.AspectMode), out is_first);
				
				if (ImageAspectRatioFitterAspectMode_EnumRef == -1)
				{
				    Utils.LoadCSTable(L, typeof(ImageAspectRatioFitter.AspectMode));
				    ImageAspectRatioFitterAspectMode_EnumRef = LuaAPI.luaL_ref(L, LuaIndexes.LUA_REGISTRYINDEX);
				}
				
            }
			
			if (LuaAPI.xlua_tryget_cachedud(L, (int)val, ImageAspectRatioFitterAspectMode_EnumRef) == 1)
            {
			    return;
			}
			
            IntPtr buff = LuaAPI.xlua_pushstruct(L, 4, ImageAspectRatioFitterAspectMode_TypeID);
            if (!CopyByValue.Pack(buff, 0, (int)val))
            {
                throw new Exception("pack fail fail for ImageAspectRatioFitter.AspectMode ,value="+val);
            }
			
			LuaAPI.lua_getref(L, ImageAspectRatioFitterAspectMode_EnumRef);
			LuaAPI.lua_pushvalue(L, -2);
			LuaAPI.xlua_rawseti(L, -2, (int)val);
			LuaAPI.lua_pop(L, 1);
        ...
        public void Get(RealStatePtr L, int index, out ImageAspectRatioFitter.AspectMode val)
        {
		    LuaTypes type = LuaAPI.lua_type(L, index);
            if (type == LuaTypes.LUA_TUSERDATA )
            {
			    if (LuaAPI.xlua_gettypeid(L, index) != ImageAspectRatioFitterAspectMode_TypeID)
				{
				    throw new Exception("invalid userdata for ImageAspectRatioFitter.AspectMode");
				}
                ...
                if (!CopyByValue.UnPack(buff, 0, out e))
                {
                    throw new Exception("unpack fail for ImageAspectRatioFitter.AspectMode");
                }
				val = (ImageAspectRatioFitter.AspectMode)e;
                
            }
            else
            {
                val = (ImageAspectRatioFitter.AspectMode)objectCasters.GetCaster(typeof(ImageAspectRatioFitter.AspectMode))(L, index, null);
            }
        }
		
        public void UpdateImageAspectRatioFitterAspectMode(RealStatePtr L, int index, ImageAspectRatioFitter.AspectMode val)
        {
            ...
            if (LuaAPI.lua_type(L, index) == LuaTypes.LUA_TUSERDATA)
            {
                ...
                if (!CopyByValue.Pack(buff, 0,  (int)val))
                {
                    throw new Exception("pack fail for ImageAspectRatioFitter.AspectMode ,value="+val);
                }
            }
	...
	public partial class StaticLuaCallbacks
    {
	    internal static bool __tryArrayGet(Type type, RealStatePtr L, ObjectTranslator translator, object obj, int index)
		{
			...
			else if (type == typeof(ImageAspectRatioFitter.AspectMode[]))
			{
			    ImageAspectRatioFitter.AspectMode[] array = obj as ImageAspectRatioFitter.AspectMode[];
				translator.PushImageAspectRatioFitterAspectMode(L, array[index]);
				return true;
			}
            return false;
		}
